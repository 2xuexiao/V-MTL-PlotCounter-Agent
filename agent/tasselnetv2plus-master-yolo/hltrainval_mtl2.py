...
from hlnet_v2pp_variety_mtl6 import *
...
def train(net, train_loader, criterion, optimizer, epoch, args):
    ...
    for i, sample in enumerate(train_loader):
        ...
        # forward
        outputs, outputs_class = net(inputs, is_normalize=False)
        # generate targets
        targets = F.conv2d(targets, target_filter, stride=os)
        # compute loss
        #print(outputs.shape)
        #print(targets.shape)
        #loss = criterion(outputs, targets) + criterion(outputs.view(outputs.size(0), -1).sum(dim=1), targets.view(outputs.size(0), -1).sum(dim=1))/outputs.size(0)/outputs.size(1)/outputs.size(2)/outputs.size(3)
        criterion_class = nn.CrossEntropyLoss(reduction='mean').cuda()
        loss = criterion(outputs, targets) + 0.01 * criterion_class(outputs_class, labels)
        ...
        running_frame_rate = args.batch_size * float(1 / (end - start + float("1e-8")))
...
def validate(net, valset, val_loader, criterion, epoch, args):
    ...
    with torch.no_grad():
        ...
        for i, sample in enumerate(val_loader):
            torch.cuda.synchronize()
            start = time()+float("1e-8")

            image, gtcount = sample['image'], sample['gtcount']
            gtclass = sample['label']
            gtclass = gtclass.numpy()
            # inference
            output, outputs_class = net(image.cuda(), is_normalize=not args.save_output)
            if args.save_output:
                output_save = output
                # normalization
            ...
            pdclass = torch.argmax(outputs_class, dim=1)
            pdclass = pdclass.cpu().numpy()
            pdcount = output.sum()
            ...
            if args.save_output:
                ...
                if args.dataset == 'mtc':
                    ...
                    #plt.tight_layout(rect=[0, 0, 1, 1]) # soybean counting
                    plt.tight_layout(rect=[0, 0, 1, 1.3])  # soybean counting
                ...
                new_image_name = re.sub(r'\.(jpg|JPG)$', '.png', image_name)
                save_dir= epoch_result_dir +"dotimage"
                if not os.path.exists(save_dir):
                    os.makedirs(save_dir)
                plt.savefig(os.path.join(save_dir, new_image_name), bbox_inches='tight', dpi = 300)
                plt.close()
                ...
                new_image_name = re.sub(r'\.(jpg|JPG)$', '.png', image_name)
                save_dir = epoch_result_dir + "output"
                if not os.path.exists(save_dir):
                    os.makedirs(save_dir)
                plt.savefig(os.path.join(save_dir, new_image_name), bbox_inches='tight', dpi=300)
                plt.close()
            ...
            pd_classes.append(pdclass)
            gt_classes.append(gtclass)
            ...
            running_frame_rate = 1 * float(1 / (end - start + float("1e-8")))
            avg_frame_rate = (avg_frame_rate*i + running_frame_rate)/(i+1)
            if i % args.print_every == args.print_every - 1:
                print(
    ...
    np.save(args.snapshot_dir + '/pd_classes.npy', pd_classes)
    np.save(args.snapshot_dir + '/gt_classes.npy', gt_classes)
